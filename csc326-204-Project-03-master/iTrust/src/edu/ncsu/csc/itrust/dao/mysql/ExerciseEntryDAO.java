...
public class ExerciseEntryDAO {
	...
	public List<ExerciseEntryBean> getPatientExerciseDiaryTotals(long patientMID)
			throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement(" SELECT EntryID AS EntryID, Date AS Date, ExerciseType AS ExerciseType, Name AS Name, "
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstring = conn
					.prepareStatement("INSERT INTO exerciseEntry(EntryID, Date, ExerciseType, "
							...
							+ " VALUES (?,?,?,?,?,?,?,?,?)"); ) {
			//pstring = 
			exerciseLoader.loadParameters(pstring, exerciseEntry);
	...
	public long getNextEntryID() throws DBException, ITrustException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(exerciseEntry.EntryID) "
					+ "FROM exerciseEntry"); ) {
	...
	public int deleteExerciseEntry(long entryID, long patientMID)
			throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("DELETE FROM exerciseEntry "
	...
	public List<ExerciseEntryBean> getBoundedExerciseDiary(Date lower,
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement("SELECT * FROM exerciseEntry "
