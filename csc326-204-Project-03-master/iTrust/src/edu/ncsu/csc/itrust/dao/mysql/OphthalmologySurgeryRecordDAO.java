...
public class OphthalmologySurgeryRecordDAO {
	...
	public void addOphthalmologySurgeryRecord(OphthalmologySurgeryRecordBean p) throws DBException {
		...
		try 
			...
			PreparedStatement ps = conn.prepareStatement("INSERT INTO ophthalmologySurgery (MID,dateVisit,"
					...
					+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			PreparedStatement ps2 = ophthalmologyLoader.loadParameters(ps, p);
			PreparedStatement ps3 = conn.prepareStatement("SELECT * FROM ophthalmologySurgery ORDER BY oid DESC limit 1");
			)
			
		{
				ps2.executeUpdate(); //HERE
	...
	public OphthalmologySurgeryRecordBean getOphthalmologySurgeryRecord(long oid) 
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ophthalmologySurgery WHERE OID" + " = ?");
			)
		{
			ps.setLong(1, oid);
	...
	public List<OphthalmologySurgeryRecordBean> 
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ophthalmologySurgery WHERE MID=? ORDER BY dateVisit DESC");
			)
		{
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();
	...
	public void editOphthalmologySurgeryRecordsRecord(long oid, OphthalmologySurgeryRecordBean p) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE ophthalmologySurgery SET MID=?,dateVisit=?,docLastName=?,docFirstName=?,vaNumOD=?,vaDenOD=?,vaNumOS=?,vaDenOS=?,sphereOD=?,"
