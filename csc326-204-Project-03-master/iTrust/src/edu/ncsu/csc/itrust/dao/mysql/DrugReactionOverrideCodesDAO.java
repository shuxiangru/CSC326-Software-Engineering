...
public class DrugReactionOverrideCodesDAO {
	...
	public List<OverrideReasonBean> getAllORCodes() throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM drugreactionoverridecodes ORDER BY CODE"); ) {
	...
	public OverrideReasonBean getORCode(String code) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM drugreactionoverridecodes WHERE Code = ?"); ) {
			ps.setString(1, code);
			ResultSet rs = ps.executeQuery();
			if (rs.next()){
	...
	public boolean addORCode(OverrideReasonBean orc) throws DBException, ITrustException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO drugreactionoverridecodes (Code, Description) " + "VALUES (?,?)"); ) {
	...
	public int updateCode(OverrideReasonBean orc) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE drugreactionoverridecodes SET Description = ? " + "WHERE Code = ?"); ) {
