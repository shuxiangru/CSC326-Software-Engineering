...
public class NDCodesDAO {
	...
	public List<MedicationBean> getAllNDCodes() throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ndcodes ORDER BY CODE");
			)
		{
	...
	public MedicationBean getNDCode(String code) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ndcodes WHERE Code = ?");
			)
		{
			ps.setString(1, code);
			ResultSet rs = ps.executeQuery();
	...
	public boolean addNDCode(MedicationBean med) throws DBException, ITrustException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO ndcodes (Code, Description) " + "VALUES (?,?)");
			)
		{
	...
	public int updateCode(MedicationBean med) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE ndcodes SET Description = ? " + "WHERE Code = ?");
			)
		{
	...
	public boolean removeNDCode(MedicationBean med) throws DBException, ITrustException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("DELETE FROM ndcodes WHERE Code=?");
			)
		{
			ps.setString(1, med.getNDCode());
			return (1 == ps.executeUpdate());
