...
public class FoodEntryDAO {
	...
	public List<FoodEntryBean> getPatientFoodDiary(long patientMID)
		...
		try (Connection conn = factory.getConnection(); 
			...
			PreparedStatement pstring = conn.prepareStatement("SELECT * FROM foodEntry WHERE "
	...
	public List<FoodEntryBean> getPatientFoodDiaryTotals(long patientMID)
		...
		try (Connection conn = factory.getConnection();
			...
			PreparedStatement pstring = conn.prepareStatement("SELECT DateEaten AS DateEaten, "
	...
	public void addFoodEntry(FoodEntryBean foodEntry) throws DBException,
		...
		try(Connection conn = factory.getConnection(); 
			...
			PreparedStatement pstring1 = conn.prepareStatement("INSERT INTO foodEntry(EntryID,"
					+ " DateEaten, MealType, FoodName, Servings, Calories, "
					+ "Fat, Sodium, Carbs, Sugar, Fiber, Protein, PatientID) "
					+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
			PreparedStatement pstring2 = foodLoader.loadParameters(pstring1, foodEntry);
			)
		{
			pstring2.executeUpdate();
		} catch (SQLException e) {
			throw new DBException(e);
		} 
	}
	...
	public long getNextEntryID() throws DBException, ITrustException {
		...
		try(Connection conn = factory.getConnection();
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(foodEntry.EntryID) "
					+ "FROM foodEntry");
			)
		{
	...
	public int deleteFoodEntry(long entryID, long patientMID)
		...
		try (Connection conn = factory.getConnection();
			...
			PreparedStatement pstmt = conn.prepareStatement("DELETE FROM foodEntry "
	...
	public int updateFoodEntry(long entryID, long patientMID,
		...
		try (Connection conn = factory.getConnection();
			...
			PreparedStatement pstmt = conn.prepareStatement("UPDATE foodEntry "
	...
	public List<FoodEntryBean> getBoundedFoodDiary(Date lower, Date upper,
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement pstring = conn.prepareStatement("SELECT * FROM foodEntry "
					+ " WHERE PatientID = ? "
