...
<%@page import="edu.ncsu.csc.itrust.action.EditOphthalmologyOVAction"%>
...
<%
	...
	if (!currentPersonnel.getSpecialty().equalsIgnoreCase("ophthalmologist") && !currentPersonnel.getSpecialty().equalsIgnoreCase("optometrist")) {
		response.sendRedirect("/iTrust/auth/hcp/viewOphthalmologyOV.jsp?oid=" + oidString);
	}
	...
	//don't run unless the form was actually submitted
		...
		//prepare to add beans!
				EditOphthalmologyOVAction editAction = new EditOphthalmologyOVAction(prodDAO, loggedInMID);
			...
			editAction.editOphthalmologyOV(bean.getOid(), ophBean);
			addedSomething = true;
			}
			
			if (addedSomething) {
				response.sendRedirect("/iTrust/auth/hcp/ophthalmologyHome.jsp?editOV");
			}
		}
		catch(FormValidationException e) {
...
<div id="mainpage" align="center">
	...
	<form <% out.print("action=\"/iTrust/auth/hcp/editOphthalmologyOV.jsp?oid=" + oidString + "\""); %> method="post" id="officeVisit" >
		<input type="hidden" name="formIsFilled" value="true" />
		<table class="fTable" align="center">
			<tr><th colspan="3">Edit Ophthalmology Office Visit</th></tr>
			<tr>
		...
		<input type="submit" id="submit" value="Submit" form="officeVisit"/>
	</form>
</div>

<form <% out.print("action=\"/iTrust/auth/hcp/editOphthalmologyOV.jsp?oid=" + oidString + "\""); %> method="post" id="removeDiagnosisForm" >
<input type="hidden" name="formIsFilled" value="true" />
...
<form <% out.print("action=\"/iTrust/auth/hcp/editOphthalmologyOV.jsp?oid=" + oidString + "\""); %> method="post" id="diagnosesForm" >
		<input type="hidden" name="formIsFilled" value="true" />
