...
public class ProceduresDAO {
	...
	public List<ProcedureBean> getList(long visitID) throws DBException {
		try (
				...
				PreparedStatement ps = conn.prepareStatement("Select * From ovprocedure,cptcodes,officevisits Where ovprocedure.VisitID = ? "
	...
	public void remove(long ovProcedureID) throws DBException {
		try (
				Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("DELETE FROM ovprocedure WHERE ID=? ");
			){
