...
<%
	pageTitle = "iTrust - Request an Appointment";
%>
...
<%
	...
	List<HCPVisitBean> visits = hcpAction.getVisitedHCPMIDs();
	...
	if (request.getParameter("request") != null) {
		...
		PersonnelBean pb = personnelDAO.getPersonnel(hcpid);
		if (pb != null) {
			if (apptType.equals("Orthopedic") &&(pb.getSpecialty() == null || !pb.getSpecialty().equals("Orthopedic"))) {
				msg = "ERROR: HCP for Orthopedic appointment must have Orthopedic Specialty";
			} else if (apptType.equals("Physical Therapy") && (pb.getSpecialty() == null || !pb.getSpecialty().equals("Physical Therapist"))) {
				msg = "ERROR: HCP for Physical Therapy appointment must be a Physical Therapist";
			} else {
				try {
					if (date.length() == 10) {
						...
						if (msg.contains("conflicts")) {
							...
							loggingAction.logEvent(TransactionType.APPOINTMENT_REQUEST_SUBMITTED, loggedInMID,
									hcpid, "");
						}
					} else {
