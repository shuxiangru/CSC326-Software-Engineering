...
public class ApptDAO {
	...
	public List<ApptBean> getApptsFor(final long mid) throws SQLException, DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement("SELECT * FROM appointment WHERE "
						+ (mid >= MIN_MID ? "doctor_id" : "patient_id") + "=? ORDER BY sched_date;");) {
	...
	public void scheduleAppt(final ApptBean appt) throws SQLException, DBException {
		try (Connection conn = factory.getConnection();
			...
			this.abloader.loadParameters(pstring, appt);
	...
	public List<ApptBean> getAllHCPConflictsForAppt(final long mid, final ApptBean appt)
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn
						.prepareStatement("SELECT * " + "FROM appointment a, appointmenttype type " + // and
		// the
		// corresponding
		// types
	...
	public List<ApptBean> getAllConflictsForDoctor(final long mid) throws SQLException, DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn
						.prepareStatement("SELECT a1.* " + "FROM appointment a1, appointment a2, " + // all
		// possible
		// sets
		// of
		// 2
