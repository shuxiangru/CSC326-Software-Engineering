...
/**
 * Tests that patients can edit and delete entries in their food diary.
 */
public class EditAndDeleteFoodDiaryTest extends iTrustSeleniumTest {
	...
	public void testEditFoodDiaryEntryValidValues() throws Exception {
		...
		driver.findElement(By.xpath("//h2[text()='My Wellness Diary']")).click();
		driver.findElement(By.linkText("My Food Diary")).click();
		assertEquals("iTrust - View My Food Diary", driver.getTitle());

		/*
		 * now find the values individually (name:row#) have to do it by value
		 * since they are input fields used for editing and deleting
		 */
		...
		/*
		 * click the edit entry button (each row in table has 14 and edit is 2nd
		 * last)
		 */
		WebElement edit = driver.findElement(By.cssSelector("input[value='Edit Entry']"));
		edit.click();
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElements(By.cssSelector("input[value='Submit']")).get(0));
	...
	/**
	 * Tests that a Patient cannot enter invalid values when editing a Food
	 * Diary entry.
	 */
	public void testEditFoodDiaryEntryInvalidValues() throws Exception {
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElements(By.cssSelector("input[value='Submit']")).get(0));
		assertTrue(driver.getPageSource().contains("Number of Servings " + "must be greater than 0"));
	...
	/**
	 * Tests that a user will be presented with a popup before completely
	 * deleting an entry.
	 */
	public void testDeleteFoodDiaryEntry() throws Exception {
		...
		/*
		 * make sure the values are right to begin with now find the values
		 * individually (name:row#) have to do it by value since they are input
		 * fields used for editing and deleting
		 */
		WebElement dateVal = driver.findElement(By.name("Date:0"));
		...
		/*
		 * htmlunitdriver doesn't work with confirms well, and this is the best
		 * workaround there is
		 */
		...
		assertEquals("09/30/2012", dateVal2.getAttribute(VALUE));
		mealVal2 = new Select(driver.findElement(By.name("MealType:0")));
		assertEquals("Lunch", mealVal2.getFirstSelectedOption().getAttribute(VALUE));
	...
	/**
	 * Tests that a user can cancel deletion of an entry and that nothing will
	 * happen.
	 */
	...
	/**
	 * Tests that a user cannot enter an invalid date when editing a food diary
	 * entry.
	 */
	public void testEditEntryWithInvalidDate() throws Exception {
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElements(By.cssSelector("input[value='Submit']")).get(0));
		assertTrue(driver.getPageSource().contains("Date Eaten: " + "MM/DD/YYYY"));
