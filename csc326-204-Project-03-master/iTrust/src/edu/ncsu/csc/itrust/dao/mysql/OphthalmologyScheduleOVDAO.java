...
public class OphthalmologyScheduleOVDAO {
	...
	public void addOphthalmologyScheduleOVRecord(OphthalmologyScheduleOVRecordBean p) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO ophthalmologySchedule(PATIENTMID,DOCTORMID,"
					+ "dateTime,docLastName,docFirstName,comments,pending,accepted)"
					+ " VALUES(?,?,?,?,?,?,?,?)");
				PreparedStatement ps2 = ophthalmologyScheduleOVLoader.loadParameters(ps, p);
				PreparedStatement ps3 = conn.prepareStatement("SELECT * FROM ophthalmologySchedule ORDER BY oid DESC limit 1");
			)
		{
			ps2.executeUpdate();
	...
	public OphthalmologyScheduleOVRecordBean getOphthalmologyScheduleOVRecord(long oid) 
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ophthalmologySchedule WHERE OID" + " = ?");
			)
		{
			ps.setLong(1, oid);
	...
	public List<OphthalmologyScheduleOVRecordBean> 
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ophthalmologySchedule WHERE PATIENTMID=? ORDER BY dateTime DESC");
			)
		{
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();
	...
	public List<OphthalmologyScheduleOVRecordBean> 
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ophthalmologySchedule WHERE DOCTORMID=? ORDER BY dateTime DESC");
			)
		{
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();
	...
	public void editOphthalmologyScheduledOVRecordsRecord(long oid, OphthalmologyScheduleOVRecordBean p) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE ophthalmologySchedule SET PATIENTMID=?,DOCTORMID=?,dateTime=?,"
