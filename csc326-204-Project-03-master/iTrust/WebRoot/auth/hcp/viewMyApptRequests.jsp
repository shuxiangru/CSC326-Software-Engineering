...
<%
	ViewApptRequestsAction viewReqAction = new ViewApptRequestsAction(loggedInMID, prodDAO);
	List<ApptRequestBean> reqs = viewReqAction.getApptRequests();
	PatientDAO pDAO = prodDAO.getPatientDAO();
	loggingAction.logEvent(TransactionType.APPOINTMENT_REQUEST_VIEW, loggedInMID, 0L, "");
%>
<h1>My Appointment Requests</h1>
<%
	String msg = "";
	if ((request.getParameter("req_id") != null) && (request.getParameter("status") != null)) {
		boolean myReq = false;
		ApptRequestBean theReq = null;
		for (ApptRequestBean req : reqs) {
			if ((req.getRequestedAppt().getApptID() + "").equals(request.getParameter("req_id"))) {
				myReq = true;
				theReq = req;
			}
		}
		...
		if (myReq) {
			...
			if ("approve".equals(request.getParameter("status"))) {
				...
				if (theReq.getRequestedAppt().getApptType().equals("Physical Therapy")) {
					PhysicalTherapyVisitAction ptAction = new PhysicalTherapyVisitAction(prodDAO,
							loggedInMID.longValue(), Long.toString(theReq.getRequestedAppt().getPatient()));
					List<PhysicalTherapyVisitBean> ptb = ptAction.getPhysicalTherapyVisitsByHCPID(
							theReq.getRequestedAppt().getPatient(), loggedInMID.longValue());
					if (ptb.size() <= 0) {
						msg = "Physical Therapy appointments can only be accepted if there is an order for that patient";
						approved = false;
					}
				}
				if (approved) {
					...
					if (!msg.contains("acted upon"))
						loggingAction.logEvent(TransactionType.APPOINTMENT_REQUEST_APPROVED, loggedInMID,
								theReq.getRequestedAppt().getPatient(), "");
				}
			} else if ("reject".equals(request.getParameter("status"))) {
				msg = viewReqAction.rejectApptRequest(reqID);
				if (!msg.contains("acted upon"))
					loggingAction.logEvent(TransactionType.APPOINTMENT_REQUEST_REJECTED, loggedInMID,
							theReq.getRequestedAppt().getPatient(), "");
			}
...
<ul style="list-style-type: none;">
	<%
		for (ApptRequestBean req : reqs) {
			if (req.getRequestedAppt().getDate().compareTo(new Timestamp(System.currentTimeMillis())) >= 0) {
				out.print("<li style=\"padding: 1em; clear: both;\">");
				out.print("<div style=\"float: left; width: 20em;\">");
				out.print("Request from: " + pDAO.getName(req.getRequestedAppt().getPatient()) + "<br />");
				out.print("Appointment type: " + req.getRequestedAppt().getApptType() + "<br />");
				SimpleDateFormat frmt = new SimpleDateFormat("MM/dd/yyy hh:mm a");
				out.print("At time: " + frmt.format(req.getRequestedAppt().getDate()) + "<br />");
				out.print("Comment: " + req.getRequestedAppt().getComment());
				out.print("</div>");
				out.print("<div style=\"float: left; margin-left: 1em;\">");
				if (req.isPending()) {
					out.print("<a href=\"viewMyApptRequests.jsp?req_id=" + req.getRequestedAppt().getApptID()
