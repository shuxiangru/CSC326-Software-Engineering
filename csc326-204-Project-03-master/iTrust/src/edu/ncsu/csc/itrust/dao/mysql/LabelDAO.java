...
public class LabelDAO {
	...
	public LabelBean getLabel(long entryID) throws DBException {
		...
		try (Connection conn = factory.getConnection();
			...
			PreparedStatement pstring = conn.prepareStatement("SELECT * FROM labels WHERE EntryID = ?");
			)
		{
	...
	public List<LabelBean> getLabels(long patientMID) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement pstring = conn.prepareStatement("SELECT * FROM labels WHERE "
	...
	public void addLabel(LabelBean label) 
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement pstring = conn.prepareStatement("INSERT INTO labels(EntryID,"
					+ " PatientID, LabelName, LabelColor) "
					+ "VALUES (?,?,?,?)");
				PreparedStatement pstring2 = labelLoader.loadParameters(pstring, label);
			)
		{
	...
	public int editLabel(long entryID, long patientMID,
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement pstmt = conn.prepareStatement("UPDATE labels "
	...
	public int deleteLabel(long entryID, long patientMID) 
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement pstmt = conn.prepareStatement("DELETE FROM labels "
					+ "WHERE EntryID = ? AND PatientID = ?");
			)
		{
