...
public class ObstetricsRecordDAO {
	...
	public void addObstetricsRecord(ObstetricsRecordBean p) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO obstetrics(MID,pregId,LMP,EDD,weeksPregnant,"
					+ "dateVisit,yearConception,hoursInLabor,deliveryType,pregnancyStatus,"
					+ "weight,bloodPressureS,bloodPressureD,FHR,FHU)"
					+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				PreparedStatement ps3 = conn.prepareStatement("SELECT * FROM obstetrics WHERE pregId=? ORDER BY oid DESC limit 1"); //it will be the largest OID in the DB
			)
		{
			PreparedStatement ps2 = obstetricsLoader.loadParameters(ps, p);
			ps2.executeUpdate();
			ps2.close();
			
			//then, set the OID of the original bean to the one the database generates
			
			ps3.setLong(1, p.getPregId());
	...
	public ObstetricsRecordBean getObstetricsRecord(long oid) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM obstetrics WHERE OID = ?");
			)
		{
			ps.setLong(1, oid);
	...
	public List<ObstetricsRecordBean> getObstetricsRecordsByMID(long mid)
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM obstetrics WHERE MID=? ORDER BY dateVisit DESC");
			)
		{
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();
	...
	public List<ObstetricsRecordBean> getObstetricsRecordsByMIDPregStatus(long mid, String pregStatus)
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM obstetrics WHERE MID=? AND pregnancyStatus=?"
	...
	public ObstetricsRecordBean getObstetricsRecordsByMIDLargestpregId(long mid)
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM obstetrics WHERE MID=? ORDER BY pregId DESC");
			)
		{
			ps.setLong(1, mid);
	...
	public void editObstetricsRecord(long oid, ObstetricsRecordBean p) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE obstetrics SET MID=?,pregId=?,LMP=?,EDD=?,weeksPregnant=?,"
					+ "dateVisit=?,yearConception=?,hoursInLabor=?,deliveryType=?,pregnancyStatus=?,"
					+ "weight=?,bloodPressureS=?,bloodPressureD=?,fhr=?,fhu=? "
					+ "WHERE OID=?");
				)
