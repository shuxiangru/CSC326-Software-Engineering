...
public class RemoteMonitoringDAO {
	...
	public List<TelemedicineBean> getTelemedicineBean(final long patientMID) throws DBException {
		...
		try (
				...
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringlists WHERE PatientMID=?");
			) {
	...
	public List<RemoteMonitoringDataBean> getPatientsData(final long loggedInMID) throws DBException {
		try (
				Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringlists WHERE HCPMID=? ORDER BY PatientMID");
	...
	public List<RemoteMonitoringDataBean> getPatientDataByDate(final long patientMID, final Date lower, final Date upper) throws DBException{
		try(
				Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringdata WHERE PatientID=? AND timeLogged >= ? AND timeLogged <= ? ORDER BY timeLogged DESC");
	...
	public List<RemoteMonitoringDataBean> getPatientDataByType(final long patientMID, final String dataType) throws DBException{
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringdata WHERE PatientID=? AND "+dataType+" != -1 ORDER BY timeLogged ASC");
				){
			pstmt.setLong(1, patientMID);
	...
	throws ITrustException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("INSERT INTO remotemonitoringdata(PatientID, height, weight, "
	...
	private int getNumberOfDailyEntries(final long patientMID, final String dataType) throws DBException{
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringdata WHERE PatientID=? AND "
	...
	public void validatePR(final long representativeMID, final long patientMID)
		...
		try (
				Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM representatives WHERE RepresenterMID=? AND RepresenteeMID=?");
			){
	...
	public List<PersonnelBean> getMonitoringHCPs(final long patientMID) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel, remotemonitoringlists "
	...
	public boolean addPatientToList(final long patientMID,
		...
		try (
				Connection conn = factory.getConnection();
				PreparedStatement pstmt1 = conn.prepareStatement("INSERT INTO remotemonitoringlists(PatientMID, HCPMID, " + permissionPS + ") VALUES(?,?,?,?,?,?,?,?)");
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM remotemonitoringlists WHERE PatientMID = ? AND HCPMID = ?");
			) {
			pstmt.setLong(1, patientMID);
			...
			if(results.next()){
				...
				results.close();
				pstmt1.setLong(1, patientMID);
				pstmt1.setLong(2, HCPMID);
				pstmt1.setBoolean(3, tBean.isSystolicBloodPressureAllowed());
				pstmt1.setBoolean(4, tBean.isDiastolicBloodPressureAllowed());
				pstmt1.setBoolean(5, tBean.isGlucoseLevelAllowed());
				pstmt1.setBoolean(6, tBean.isHeightAllowed());
				pstmt1.setBoolean(7, tBean.isWeightAllowed());
				pstmt1.setBoolean(8, tBean.isPedometerReadingAllowed());
	...
	public boolean removePatientFromList(final long patientMID, final long HCPMID) throws DBException {
		...
		try(
				Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("DELETE FROM remotemonitoringlists WHERE PatientMID = ? AND HCPMID = ?");
				) {
