...
public class SleepEntryDAO {
	...
	public List<SleepEntryBean> getPatientSleepDiaryTotals(long patientMID)
		...
		try (
				Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement(" SELECT EntryID AS EntryID, Date AS Date, SleepType AS SleepType, "
	...
	public void addSleepEntry(SleepEntryBean sleepEntry)
		...
		try (Connection conn = factory.getConnection();
			...
			sleepLoader.loadParameters(pstring, sleepEntry);
	...
	public long getNextEntryID() throws DBException, ITrustException {
		...
		try (
				...
				PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(sleepEntry.EntryID) "
						+ "FROM sleepEntry");
			) {
	...
	public int deleteSleepEntry(long entryID, long patientMID)
		...
		try (
				Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("DELETE FROM sleepEntry "
	...
	public List<SleepEntryBean> getBoundedSleepDiary(Date lower,
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement pstring = conn.prepareStatement("SELECT * FROM sleepEntry "
