...
public class OfficeVisitDAO {
	...
	public long add(OfficeVisitBean ov) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO officevisits (VisitDate, Notes, HCPID, PatientID, HospitalID, IsERIncident, appt_type) VALUES (?,?,?,?,?,?,?)");
			)
		{
			setValues(ps, ov);
	...
	public void update(OfficeVisitBean ov) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE officevisits SET VisitDate=?, Notes=?, HCPID=?, "
	...
	public OfficeVisitBean getOfficeVisit(long visitID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("Select * From officevisits Where ID = ?");
			)
		{
	...
	public boolean checkOfficeVisitExists(long ovID, long pid) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM officevisits WHERE ID=? AND PatientID=?");
			)
		{
			ps.setLong(1, ovID);
			ps.setLong(2, pid);
	...
	public List<OfficeVisitBean> getOfficeVisitsFromHospital(String hospitalID) throws DBException{
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM officevisits WHERE hospitalID = ? ORDER BY ID DESC");
			)
		{
