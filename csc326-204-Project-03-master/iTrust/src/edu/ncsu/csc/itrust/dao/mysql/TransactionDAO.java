...
public class TransactionDAO {
	...
	public OperationalProfile getOperationalProfile() throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT TransactionCode, count(transactionID) as TotalCount, "
	...
	private List<TransactionBean> addAndSortRoles(List<TransactionBean> tbList, long patientID, boolean sortByRole) throws DBException {
		...
		try (Connection conn = factory.getConnection();){
			...
			for(TransactionBean t : tbList) {
				try(PreparedStatement ps = conn.prepareStatement("SELECT Role FROM users WHERE MID=?");){
				...
				if(role.equals("er"))
					...
					role = "LHCP";
					try(PreparedStatement p2 = conn.prepareStatement("SELECT PatientID FROM declaredhcp WHERE HCPID=?");){
					
						p2.setLong(1, t.getLoggedInMID());
					...
					role = "Patient";
					try(PreparedStatement p3 = conn.prepareStatement("SELECT representeeMID FROM representatives WHERE representerMID=?");){
					p3.setLong(1, t.getLoggedInMID());
