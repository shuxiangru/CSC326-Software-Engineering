...
public class HospitalsDAO {
	...
	public List<HospitalBean> getAllHospitals() throws DBException {
		...
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM hospitals ORDER BY HospitalName");
			)
		{
	...
	public HospitalBean getHospital(String id) throws DBException {
		...
		try  (Connection conn = factory.getConnection();
			...
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM hospitals WHERE HospitalID = ?");
			)
		{
			ps.setString(1, id);
			ResultSet rs = ps.executeQuery();
	...
	public boolean addHospital(HospitalBean hosp) throws DBException, ITrustException {
		...
		try  (Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("INSERT INTO hospitals (HospitalID, HospitalName, Address, City, State, Zip) " + "VALUES (?,?,?,?,?,?)");
			)
		{
	...
	public int updateHospital(HospitalBean hosp) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("UPDATE hospitals SET HospitalName=?,Address=?,City=?,State=?,Zip=?" + "WHERE HospitalID = ?");
			)
		{
	...
	public boolean assignHospital(long hcpID, String hospitalID) throws DBException, ITrustException {
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("INSERT INTO hcpassignedhos (HCPID, HosID) VALUES (?,?)");
	...
	public boolean removeHospitalAssignment(long hcpID, String hospitalID) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("DELETE FROM hcpassignedhos WHERE HCPID = ? AND HosID = ?");
			)
		{
			ps.setLong(1, hcpID);
	...
	public int removeAllHospitalAssignmentsFrom(long hcpID) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("DELETE FROM hcpassignedhos WHERE HCPID = ?");
			)
		{
			ps.setLong(1, hcpID);
			int check = ps.executeUpdate();
	...
	public boolean checkLTHasHospital(long hcpID) throws DBException{
		...
		try(Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM hcpassignedhos WHERE HCPID = ?");
			)
		{
			ps.setLong(1, hcpID);
			ResultSet rs = ps.executeQuery();
	...
	public List<HospitalBean> getAllPatientHospitals(long pid) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement ps = conn.prepareStatement("SELECT DISTINCT hospitals.* FROM personalhealthinformation " +
	...
	public List<HospitalBean> getHospitalsAssignedToPhysician(long pid) throws SQLException
	{
		...
		HospitalBeanLoader loader = new HospitalBeanLoader();
 		try(Connection conn = factory.getConnection();
 				PreparedStatement ps = conn.prepareStatement("Select hosID from hcpassignedhos where HCPID=?");
 				PreparedStatement ps2 = conn.prepareStatement("select * from hospitals where hospitals.HospitalID=?");
 				)
		{
			...
			for (String string : hosID) {
				...
				ps2.setString(1, String.valueOf(string));
				rs = ps2.executeQuery();
				while(rs.next())
				{
