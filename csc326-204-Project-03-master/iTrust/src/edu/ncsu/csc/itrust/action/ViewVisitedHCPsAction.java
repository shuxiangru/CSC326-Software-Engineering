...
public class ViewVisitedHCPsAction {
	...
	private OrthopedicVisitDAO orthopedicDAO;
	private PhysicalTherapyVisitDAO physicalTherapyDAO;
	private SurgicalOrthopedicVisitDAO surgicalOrthopedicDAO;
	...
	public ViewVisitedHCPsAction(DAOFactory factory, long loggedInMID) {
		...
		orthopedicDAO = factory.getOrthopedicVisitDAO();
		physicalTherapyDAO = factory.getPhysicalTherapyVisitDAO();
		surgicalOrthopedicDAO = factory.getSurgicalOrthopedicVisitDAO();
	...
	private HCPVisitBean makeHCPVisitBean(PersonnelBean pb, String visitDate) throws DBException {
		...
		visitBean.setHCPAddr(pb.getStreetAddress1() + " " + pb.getStreetAddress2() + " " + pb.getCity() + ", "
				+ pb.getState() + " " + pb.getZip());
		visitBean.setDesignated(patientDAO.checkDeclaredHCP(patientMID, hcpid));
		return visitBean;
	}

	private HCPVisitBean makeHCPVisitBean(OrthopedicVisitBean ovb, String visitDate) throws DBException {
		long hcpid = ovb.getOrthopedicID();
		HCPVisitBean visitBean = new HCPVisitBean();
		visitBean.setHCPMID(hcpid);
		try {
			visitBean.setHCPName(docDAO.getName(ovb.getOrthopedicID()));
	...
	private HCPVisitBean makeHCPVisitBean(PhysicalTherapyVisitBean ptvb, String visitDate) throws DBException {
		long hcpid = ptvb.getPhysicalTherapistID();
		HCPVisitBean visitBean = new HCPVisitBean();
		visitBean.setHCPMID(hcpid);
		try {
			visitBean.setHCPName(docDAO.getName(ptvb.getPhysicalTherapistID()));
		} catch (ITrustException e) {
		}
		visitBean.setOVDate(visitDate);
		return visitBean;
	...
	private HCPVisitBean makeHCPVisitBean(SurgicalOrthopedicVisitBean sovb, String visitDate) throws DBException {
		long hcpid = sovb.getOrthopedicID();
		HCPVisitBean visitBean = new HCPVisitBean();
		visitBean.setHCPMID(hcpid);
		try {
			visitBean.setHCPName(docDAO.getName(sovb.getOrthopedicID()));
		} catch (ITrustException e) {
		}
		visitBean.setOVDate(visitDate);
		return visitBean;
	 ...
	 * @param lastName
	 *            The last name to check against. May be null or the empty
	 *            string to ignore.
	 * @param specialty
	 *            The specialty to check against. May be null or the empty
	 *            string to ignore.
	 * @param zip
	 *            The zip code to check against. May be null or the empty string
	 *            to ignore.
	 * @return true if the PersonnelBean matches all the given parameters, or
	 *         false otherwise.
	 */
	...
	/**
	 * Get a list of all HCPs visited and/or designated by by the current user.
	 * The list can optionally be filtered by the doctor's last name, specialty,
	 * or zip code.
	 * 
	 * @param lastName
	 *            The last name (or a part of it) of the doctor to search for,
	 *            or null or an empty string to accept all doctors.
	 * @param specialty
	 *            The specialty of the doctor to search for, or null or an empty
	...
	private List<HCPVisitBean> getAllVisitedHCPMIDs(String lastName, String specialty, String zip) throws ITrustException {
		// Visited HCPs in this case includes both HCPs visited *and* HCPs
		// designated by the patient. These two groups are retrieved in
		// different ways, then combined.
		List<HCPVisitBean> hcpVisits = new ArrayList<HCPVisitBean>();
		try {
			List<OfficeVisitBean> ovlist = visitDAO.getAllOfficeVisits(patientMID);
			// get most recent office visit for each provider
			LinkedHashMap<Long, OfficeVisitBean> mostRecentVisits = new LinkedHashMap<Long, OfficeVisitBean>();
		...
		List<OrthopedicVisitBean> orthopedicVisits = orthopedicDAO.getAllOrthopedicVisits(patientMID);
		for(OrthopedicVisitBean ovb : orthopedicVisits){
			boolean contains = false;
			for(HCPVisitBean hcpvb : hcpVisits)
				if(hcpvb.getHCPMID() == ovb.getOrthopedicID())
					contains = true;
			if(!contains){
				HCPVisitBean visitBean = makeHCPVisitBean(ovb, "");
				hcpVisits.add(visitBean);
			}
		}
		
		List<PhysicalTherapyVisitBean> physicalTherapyVisits = physicalTherapyDAO.getAllPhysicalTherapyVisits(patientMID);
		for(PhysicalTherapyVisitBean ptvb : physicalTherapyVisits){
			boolean contains = false;
			for(HCPVisitBean hcpvb : hcpVisits)
				if(hcpvb.getHCPMID() == ptvb.getOrthopedicVisitID())
					contains = true;
			if(!contains){
				HCPVisitBean visitBean = makeHCPVisitBean(ptvb, "");
				hcpVisits.add(visitBean);
			}
		}
		
		List<SurgicalOrthopedicVisitBean> surgicalOrthopedicVisits = surgicalOrthopedicDAO.getAllSurgicalOrthopedicVisits(patientMID);
		for(SurgicalOrthopedicVisitBean sovb : surgicalOrthopedicVisits){
			boolean contains = false;
			for(HCPVisitBean hcpvb : hcpVisits)
				if(hcpvb.getHCPMID() == sovb.getOrthopedicVisitID())
					contains = true;
			if(!contains){
				HCPVisitBean visitBean = makeHCPVisitBean(sovb, "");
				hcpVisits.add(visitBean);
			}
		}
	...
	public List<HCPVisitBean> getVisitedHCPMIDs() {
		List<HCPVisitBean> visits;
		try {
			visits = getAllVisitedHCPMIDs(null, null, null);
		} catch (ITrustException ie) {
			visits = new ArrayList<HCPVisitBean>();
		}
		...
		if (!match) {
			...
			for (PersonnelBean ele : doclist) {
				if (0 == name.compareTo(ele.getFullName())) {
					...
					visitBean.setHCPAddr(ele.getStreetAddress1() + " " + ele.getStreetAddress2() + " " + ele.getCity()
							+ ", " + ele.getState() + " " + ele.getZip());
	 ...
	 * 
	 * @param specialty
	 *            Filter by specialty. May be null or the empty string to
	 *            ignore.
	 * @param zip
