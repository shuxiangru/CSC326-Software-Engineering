...
public class AdverseEventDAO {
	...
	public List<AdverseEventBean> getReportsFor(final long mid) throws DBException, SQLException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn
						.prepareStatement("SELECT * FROM adverseevents WHERE PatientMID = ?");) {
	...
	public long removeReport(final int reportID) throws DBException, SQLException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn
						.prepareStatement("UPDATE adverseevents SET Status = ? WHERE id = ?");) {
	...
	public List<AdverseEventBean> getUnremovedAdverseEventsByCode(final String code) throws DBException, SQLException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn
						.prepareStatement("SELECT * FROM adverseevents WHERE code=? AND status=?");) {
	 ...
	 * @throws SQLException
	 */
	public List<AdverseEventBean> getPerscriptions(final String start, final String end)
			throws ParseException, DBException, SQLException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement(
						"SELECT * FROM adverseevents,ndcodes WHERE adverseevents.TimeLogged >= ? AND adverseevents.TimeLogged <= ? AND ndcodes.Code=adverseevents.Code ORDER BY adverseevents.Code, adverseevents.TimeLogged DESC");) {
	 ...
	 * @throws SQLException
	 */
	public List<AdverseEventBean> getImmunizations(final String start, final String end)
			throws ParseException, DBException, SQLException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement(
						"SELECT * FROM adverseevents,cptcodes WHERE adverseevents.TimeLogged >= ? AND adverseevents.TimeLogged <= ? AND cptcodes.Code=adverseevents.Code ORDER BY adverseevents.Code, adverseevents.TimeLogged DESC");) {
