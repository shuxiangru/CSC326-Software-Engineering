...
public class ReviewsDAO {
  ...
  public boolean addReview(ReviewsBean bean) throws DBException{
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO reviews(mid, pid, reviewdate, descriptivereview, rating, title)  VALUES (?,?,(CURRENT_TIMESTAMP),?,?,?)");
				){
  ...
  public boolean isRateable(long mid, long pid) throws DBException{
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM appointment WHERE patient_id =? AND doctor_id=?");
				){
		...
		if(!isRateable){
			try (
					...
					PreparedStatement ps = conn.prepareStatement("SELECT * FROM officevisits WHERE PatientID=? AND HCPID=?");
				){
				ps.setLong(1, mid);
				ps.setLong(2, pid);
				rs = ps.executeQuery();
