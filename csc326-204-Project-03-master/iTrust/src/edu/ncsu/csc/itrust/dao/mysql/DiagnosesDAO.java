...
public class DiagnosesDAO {
	...
	public DiagnosisStatisticsBean getDiagnosisCounts(String icdCode, String zipCode, java.util.Date lower, java.util.Date upper) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement ps1 = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND zip=? AND visitDate >= ? AND visitDate <= ? ");
				PreparedStatement ps2 = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND zip LIKE ? AND visitDate >= ? AND visitDate <= ? ");) {
			ps1.setString(1, icdCode);
			ps1.setString(2, zipCode);
			...
			ps2.setString(2, zipCode.substring(0, 3) + "%");
			ps2.setTimestamp(3, new Timestamp(lower.getTime()));
			// add 1 day's worth to include the upper
			ps2.setTimestamp(4, new Timestamp(upper.getTime() + 1000L * 60L * 60 * 24L));
	...
	public long add(DiagnosisBean bean) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO ovdiagnosis " +
	...
	public long edit(DiagnosisBean bean) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE ovdiagnosis " +
					"SET VisitID=?, ICDCode=? " +
					"WHERE ID = ?"); ) {
			ps.setLong(1, bean.getVisitID());
			ps.setString(2, bean.getICDCode());
