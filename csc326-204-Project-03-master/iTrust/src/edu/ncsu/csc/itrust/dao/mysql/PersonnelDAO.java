...
public class PersonnelDAO {
	...
	public String getName(final long mid) throws ITrustException, DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT firstName, lastName FROM personnel WHERE MID=?");
			)
		{
			pstmt.setLong(1, mid);
			ResultSet results;
	...
	public long getNextID(final Role role) throws DBException, ITrustException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(users.mid) FROM users WHERE mid BETWEEN ? AND ?");
			)
		{
	...
	public long addEmptyPersonnel(final Role role) throws DBException, ITrustException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("INSERT INTO personnel(MID, Role) VALUES(?,?)");
			)
		{
	...
	public PersonnelBean getPersonnel(final long mid) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel WHERE MID = ?");
			)
		{
			pstmt.setLong(1, mid);
			final ResultSet results = pstmt.executeQuery();
	...
	public void editPersonnel(final PersonnelBean pBean) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("UPDATE personnel SET AMID=?,firstName=?,lastName=?,"
	...
	public List<HospitalBean> getHospitals(final long mid) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM hcpassignedhos hah,hospitals h "
	...
	public List<HospitalBean> getUAPHospitals(final long mid) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT DISTINCT hospitals.* FROM hcpassignedhos hah " +
	...
	public List<PersonnelBean> getPersonnelFromHospital(final String hosid, final String specialty) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM hcpassignedhos hah inner join personnel p where hah.hosID = ? and hah.HCPID = p.MID " + (specialty.equalsIgnoreCase("all") ? "" : "and p.specialty = ? ") + "and p.role = 'hcp'");
				)
		{
	...
	public List<PersonnelBean> getPersonnelFromHospital(final String hosid) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM hcpassignedhos hah inner join personnel p where hah.hosID = ? and hah.HCPID = p.MID and p.role = 'hcp'");
			)
		{
			pstmt.setString(1, hosid);
	...
	public List<PersonnelBean> getAllPersonnel() throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel where role in ('hcp','uap','er') ");
			)
		{
			final ResultSet results = pstmt.executeQuery();
			final List<PersonnelBean> loadlist = personnelLoader.loadList(results);
	...
	public List<PersonnelBean> getUAPsForHCP(final long hcpid) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel WHERE MID IN (SELECT UAP FROM hcprelations WHERE HCP=?)");
			)
		{
	...
	public PersonnelBean getPrescribingDoctor(final PrescriptionBean prescription) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel WHERE MID IN (SELECT HCPID FROM officevisits WHERE ID=?)");
			)
		{
	...
	public List<PersonnelBean> searchForPersonnelWithName(final String first, final String last) throws DBException {
		...
		if ("%".equals(first) && "%".equals(last)) {
			...
			try (Connection conn = factory.getConnection();
					PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel WHERE firstName LIKE ? AND lastName LIKE ?");
				)
			{
	...
	public List<PersonnelBean> getLabTechs() throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM personnel WHERE role = 'lt' ");
			)
		{
			final ResultSet results = pstmt.executeQuery();
			final List<PersonnelBean> loadlist = personnelLoader.loadList(results);
	...
	public List<PersonnelBean> fuzzySearchForExpertsWithName(String first, String last) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM personnel WHERE firstName LIKE ? AND lastName LIKE ? AND role='hcp'");
			)
		{
