...
public class WardDAO {
	...
	public List<WardBean> getAllWardsByHospitalID(String id) throws DBException{
		try (
				...
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM WARDS WHERE InHospital = ? ORDER BY RequiredSpecialty");
			){
	...
	public boolean addWard(WardBean ward) throws DBException, ITrustException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO Wards (RequiredSpecialty, InHospital) " + "VALUES (?,?)");){
	...
	public int updateWard(WardBean ward) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE wards SET RequiredSpecialty=?, InHospital=? " + "WHERE WardID = ?");
				){
	...
	public List<WardRoomBean> getAllWardRoomsByWardID(long id) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM WARDROOMS WHERE InWard = ? ORDER BY RoomName");) {
	...
	public boolean addWardRoom(WardRoomBean wardRoom) throws DBException, ITrustException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO WardRooms (InWard, RoomName, Status) " + "VALUES (?,?,?)");){
	...
	public int updateWardRoom(WardRoomBean wardRoom) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE wardrooms SET InWard=?, RoomName=?, Status=? " + "WHERE RoomID = ?");) {
	...
	public List<WardBean> getAllWardsByHCP(long id) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM HCPAssignedToWard haw INNER JOIN Wards w WHERE HCP = ? AND haw.ward = w.wardid ORDER BY RequiredSpecialty");
				){
			ps.setLong(1, id);
			ResultSet rs = ps.executeQuery();
	...
	public List<PersonnelBean> getAllHCPsAssignedToWard(long id) throws DBException {
		try (
				Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM HCPAssignedToWard haw INNER JOIN Personnel p WHERE haw.HCP = p.MID AND WARD = ? ORDER BY lastName");
	...
	public boolean removeWard(long hcpID, long wardID) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("DELETE FROM HCPAssignedToWard WHERE Ward = ? and hcp = ?");
				) {
	...
	public int updateWardRoomOccupant(WardRoomBean wardRoom) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("UPDATE wardRooms SET OccupiedBy=? " + "WHERE RoomID = ?");
				){
	...
	public List<WardRoomBean> getWardRoomsByStatus(String status, Long hcpID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM wardrooms wr inner join hcpassignedtoward hw where wr.status = ? and wr.inward = hw.ward and hw.hcp = ?");
				){
	...
	public HospitalBean getHospitalByWard(String wardRoomID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM hospitals h inner join wards ward inner join wardrooms room where room.RoomID = ? and room.inward = ward.wardid and ward.inhospital = h.hospitalid");
				){
			ps.setString(1, wardRoomID);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
	...
	public boolean checkOutPatientReason(long mid, String reason) throws DBException {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO WardRoomCheckout (PID, Reason) Values(?,?)");){
	...
	public HospitalBean getHospitalByPatientID(long pid) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM hospitals h inner join wards ward inner join wardrooms room where room.OccupiedBy = ? and room.inward = ward.wardid and ward.inhospital = h.hospitalid");
				){
			ps.setLong(1, pid);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
