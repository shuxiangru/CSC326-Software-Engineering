...
public class FakeEmailDAO {
	...
	public List<Email> getAllEmails() throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM fakeemail ORDER BY AddedDate DESC"); ) {
	...
	public List<Email> getEmailsByPerson(String email) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM fakeemail WHERE ToAddr LIKE ? ORDER BY AddedDate DESC"); ) {
	...
	public void sendEmailRecord(Email email) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO fakeemail (ToAddr, FromAddr, Subject, Body) "
					+ "VALUES (?,?,?,?)"); ) {
	...
	public List<Email> getEmailWithBody(String bodySubstring) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM fakeemail WHERE Instr(Body,?)>0 ORDER BY AddedDate DESC"); ) {
