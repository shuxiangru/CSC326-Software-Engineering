...
public class PrescriptionsDAO {
	...
	public List<PrescriptionBean> getList(long visitID) throws DBException {
		try (
				...
				PreparedStatement ps = conn.prepareStatement("Select * From ovmedication,ndcodes Where ovmedication.VisitID = ? "
	...
	public long add(PrescriptionBean pres) throws DBException {
		try (
				Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("INSERT INTO ovmedication (VisitID,NDCode,StartDate,EndDate,Dosage,Instructions,OverrideOther) VALUES (?,?,?,?,?,?,?)");
				){
	...
	public void remove(long ovMedicationID) throws DBException {
		try (
				Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("DELETE FROM ovmedication WHERE ID=? ");
			) {
